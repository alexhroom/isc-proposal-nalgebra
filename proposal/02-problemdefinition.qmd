...
Performant linear algebra in R is underdeveloped. The base R matrix capability (array types and mostly wrappers for LAPACK) is simplistic and loose, and other options such as the [`rray` library](https://rray.r-lib.org/), based on on the C++ library `xtensor`, have not been updated in years.

This is an issue for users in many areas of science where performant linear algebra is important, for example for solving differential equations, or algorithms which require large amounts of matrix decomposition such as nested sampling algorithms in Bayesian model selection.

Other user-friendly programming languages such as Python (using numpy/scipy) or MATLAB offer much better flexibility, robustness, and performance than the current offerings in R. Scientists who would otherwise use R find themselves straddling R and Python, using Python for their data analysis and then R for their statistics and graphs.
...
However, the first problem - the state of linear algebra in R - can make it difficult to create effective R-Rust interfaces for packages which rely heavily on linear algebra. Developers struggle to write bindings between R and Rust for their packages, to a point which essentially prohibits this from being done. 